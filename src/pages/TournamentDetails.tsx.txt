    ...
    <div className="pb-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 text-center">
          ...
          <p className="text-xs text-neutral-400">{tournament.type === 'per_kill' ? 'Per Kill' : 'Prize Pool'}</p>
          <p className="font-bold text-lg">₹{tournament.type === 'per_kill' ? tournament.perKillPrize : tournament.prizePool}</p>
        </div>
        <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 text-center">
      ...
      {tournament.type === 'per_kill' ? (
        <div className="bg-neutral-900 border border-emerald-500/30 rounded-xl p-5 mb-6 bg-gradient-to-br from-neutral-900 to-emerald-950/20">
          <h3 className="font-bold mb-2 flex items-center gap-2 text-emerald-400">
            <span className="text-xl">🎯</span> Bounty Hunter Format
          </h3>
          <p className="text-sm text-neutral-300 leading-relaxed">
            This tournament rewards you for your aggressive gameplay. There is no fixed prize pool for survival rankings. Instead, you earn <span className="font-bold text-emerald-400">₹{tournament.perKillPrize} for every single kill</span> you secure during the match!
          </p>
        </div>
      ) : (
