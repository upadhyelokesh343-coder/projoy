...
const getRegisteredUsersFromStorage = (): User[] => {
  try {
    const stored = localStorage.getItem('projoy_registered_users');
...
const saveRegisteredUserToStorage = (user: User) => {
  try {
    const existing = getRegisteredUsersFromStorage();
    const index = existing.findIndex(u => (u.email && user.email && u.email.toLowerCase() === user.email.toLowerCase()) || u.id === user.id);
    if (index >= 0) {
      existing[index] = { ...existing[index], ...user };
    } else {
      existing.push(user);
    }
    localStorage.setItem('projoy_registered_users', JSON.stringify(existing));
  } catch (e) {
    console.warn("Could not save user to localStorage registered_users:", e);
  }
};
...
export const getBackendUrl = (path: string) => {
  if (typeof window !== "undefined") {
    ...
    // If running on Vercel, hit the Cloud Run backend absolutely to bypass missing vercel.json rewrites or misconfigurations!
    if (host.includes("vercel.app") || host.includes("localhost") || host.includes("127.0.0.1")) {
      return `https://ais-dev-wffqsfdcyvoujibjqaj4jk-614342679965.asia-east1.run.app${path}`;
    }
  }
  return path;
};
...
export const useStore = create<AppState>()((set, get) => ({
  ...
  login: async ({ phone, name, email, password, referredBy }) => {
    ...
    const localExisting = getRegisteredUsersFromStorage().find(u => u.email?.toLowerCase() === cleanedEmail);
    if (localExisting) {
      throw new Error("An account with this email already exists. Please log in.");
    }
    ...
    saveRegisteredUserToStorage(user);
    set(state => ({
      currentUser: user,
  ...
  loginWithEmailAndPassword: async (email, password) => {
    ...
    try {
      ...
      const docSnap = await getDoc(userRef);
      if (docSnap.exists()) {
        existingUserData = { id: docSnap.id, ...docSnap.data() } as User;
      }
    } catch (err: any) {
      authError = err;
    ...
    // 2. Query Firestore by email if doc by UID wasn't found directly
    if (!existingUserData) {
      try {
        const usersRef = collection(db, 'users');
        const q = query(usersRef, where('email', '==', cleanedEmail));
    ...
    // 3. Search local store state and localStorage registered users database
    if (!existingUserData) {
      const stateUsers = get().users;
      const foundInState = stateUsers.find(u => u.email?.toLowerCase() === cleanedEmail);
      if (foundInState) {
        existingUserData = foundInState;
      } else {
        const localRegistered = getRegisteredUsersFromStorage().find(u => u.email?.toLowerCase() === cleanedEmail);
        if (localRegistered) {
          existingUserData = localRegistered;
        }
      }
    }

    // 4. If user does NOT exist in registered database/localStorage, block login immediately!
    if (!existingUserData) {
      throw new Error("Please sign up first");
    }

    // 5. Verify password if auth didn't already verify it
    if (authError && authError.code !== 'auth/operation-not-allowed') {
      if (authError.code === 'auth/user-not-found' || authError.code === 'auth/invalid-credential') {
        if (existingUserData.password && existingUserData.password !== password) {
          throw new Error("Incorrect email or password. Please try again.");
        }
      }
    } else if (existingUserData.password && existingUserData.password !== password) {
      throw new Error("Incorrect email or password. Please try again.");
    }

    // 6. Log in user with their actual stored profile & balance (no extra credits added)
    if (cleanedEmail === 'rajsjarma8@gmail.com' || cleanedEmail === 'admin@nexus.com') {
      existingUserData.role = 'admin';
    }

    existingUserData.balance = Number(existingUserData.balance ?? 0);

    set({ currentUser: existingUserData });
    localStorage.setItem('projoy_currentUser', JSON.stringify(existingUserData));
    saveRegisteredUserToStorage(existingUserData);
  },
  ...
  createBondPayOrder: async (amount: number) => {
    ...
    try {
      const res = await fetch(getBackendUrl('/api/bondpay/create-order'), {
        method: 'POST',
      ...
      try {
        ...
        console.error('Failed to parse gateway JSON response:', parseErr, 'Raw response text:', rawText);
        if (!rawText) {
          return { success: false, message: 'Server returned an empty response. Please check if the backend server is active.' };
        }
        if (rawText.trim().startsWith('<')) {
          return { 
            success: false, 
            message: `Gateway server returned HTML instead of JSON (HTTP ${res.status}). This often means Vercel rewrites or Cloud Run is cold-starting/updating. Please wait 10 seconds and try again.` 
          };
        }
        return { success: false, message: `Invalid response format (HTTP ${res.status}): ${rawText.slice(0, 100)}` };
      }
  ...
  syncBondPayStatus: async (orderId: string) => {
    try {
      ...
      const res = await fetch(getBackendUrl(`/api/bondpay/check-status/${encodeURIComponent(orderId)}`));
      let data: any = {};
      try {
  ...
  updateTransactionStatus: async (transactionId, status) => {
    ...
    try {
      fetch(getBackendUrl('/api/bondpay/admin-update-status'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
